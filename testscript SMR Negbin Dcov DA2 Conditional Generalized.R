...
source("NimbleModel SMR Negbin Dcov DA2 Conditional Generalized.R")
source("NimbleFunctions SMR Negbin Dcov DA2 Conditional Generalized.R")
...
tlocs <- 25 #number of telemetry locs/marked individual.
...
data <- sim.SMR.Dcov.Generalized(D.beta0=D.beta0,D.beta1=D.beta1,res=res,
                                 ...
                                 lam0=lam0,phi=phi,p0=p0,sigma=sigma,K.mark=K.mark,K.sight=K.sight,
...
Niminits <- list(y.true=nimbuild$y.true,y.event=nimbuild$y.event,capcounts=rowSums(nimbuild$y.true),
                 ...
                 p0=inits$p0,lam0=inits$lam0,sigma=inits$sigma,phi=inits$phi)
...
parameters <- c('D0','lambda.N','p0','lam0','sigma','phi','theta.marked','theta.unmarked',
                'N','D.beta1',"n.cap")
...
config.nodes <- c('p0','lam0','sigma','phi','theta.marked','theta.unmarked[2:3]')
...
calcNodes <- c(N.node,z.nodes,pd.nodes,lam.nodes,y.mark.nodes,y.sight.nodes)
