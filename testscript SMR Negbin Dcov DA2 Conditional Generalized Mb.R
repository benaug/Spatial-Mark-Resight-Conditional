...
#y[i,j,k] ~ Negbin(mu=lam[i,j,k],size=phi), equivalent to
#y[i,j,k] ~ Negbin(prob=p[i,j,k],size=phi), p[i,j,k] = phi/(phi + lam[i,j,k]) 
...
source("NimbleModel SMR Negbin Dcov DA2 Conditional Generalized Mb.R")
source("NimbleFunctions SMR Negbin Dcov DA2 Conditional Generalized Mb.R")
...
lam0 <- 0.25 #sighting process lam0
phi <- 0.1 #count dispersion parameter. Lower values mean more overdispersion. NB converges to Poisson as phi -> Inf
...
obstype <- "negbin" #for sighting process. can also simulate "poisson"
...
data <- sim.SMR.Dcov.Generalized.Mb(D.beta0=D.beta0,D.beta1=D.beta1,res=res,
                                 ...
                                 lam0=lam0,p0.p=p0.p,p0.c=p0.c,phi=phi,sigma=sigma,K.mark=K.mark,K.sight=K.sight,
...
table(data$y) #look at distribution of i-j-k counts to see if plausible.
#also consider if overdispersion large enough for phi to be identifiable
...
inits <- list(p0.p=0.25,p0.c=0.5,lam0=1,sigma=1,phi=5,
              theta.marked=theta.marked.init,theta.unmarked=theta.unmarked.init)

#This function structures the simulated data to fit the model in Nimble (some more restructing below)
#Also checks some inits
nimbuild <- init.SMR.Dcov.Generalized.Mb(data,inits,M=M,obsmod="negbin")
...
Niminits <- list(y.true=nimbuild$y.true,y.event=nimbuild$y.event,capcounts=rowSums(nimbuild$y.true),
                 ...
                 N=sum(nimbuild$z),theta.marked=c(0.8,0.1,0.1),theta.unmarked=c(0,0.5,0.5),
                 p0.p=inits$p0.p,p0.c=inits$p0.c,lam0=inits$lam0,sigma=inits$sigma,phi=inits$phi)
...
# constants <- list(n.marked=n.marked,M=M,n.samples=nimbuild$n.samples,
...
#                 X.mark=as.matrix(X.mark),X.sight=as.matrix(X.sight),capcounts.ID=capcounts.ID)
...
Nimdata <- list(y.mark.p=nimbuild$y.mark.p, #marking process, first captures
                y.mark.c=nimbuild$y.mark.c, #marking process, subsequent captures
                dummy.data=dummy.data,cells=data$cells,InSS=data$InSS,
                X.mark=as.matrix(data$X.mark),X.sight=as.matrix(data$X.sight),
                locs=data$locs,capcounts.ID=capcounts.ID)

# set parameters to monitor
parameters <- c('D0','lambda.N','p0.p','p0.c','lam0','sigma','phi','theta.marked','theta.unmarked',
                'N','D.beta1',"n.cap")
...
config.nodes <- c('p0.p','p0.c','lam0','sigma','phi','theta.marked','theta.unmarked[2:3]')
conf <- configureMCMC(Rmodel,monitors=parameters, thin=2,
                      monitors2=parameters2,thin2=10,nodes=config.nodes)

#Add y/ID update
conf$addSampler(target = paste0("y.true[1:",M,",1:",J.sight,"]"),
                type = 'IDSampler',control = list(n.marked=n.marked,M=M,J.sight=J.sight,
...
y.sight.nodes <- Rmodel$expandNodeNames("y.true")
N.node <- Rmodel$expandNodeNames("N")
z.nodes <- Rmodel$expandNodeNames("z")
calcNodes <- c(N.node,z.nodes,pd.p.nodes,pd.c.nodes,lam.nodes,y.mark.p.nodes,y.mark.c.nodes,y.sight.nodes)
conf$addSampler(target = paste("N"),
                type = 'zSampler',control = list(z.ups=z.ups,J.sight=J.sight,n.marked=n.marked,M=M,N.node=N.node,
                                                 pd.p.nodes=pd.p.nodes,pd.c.nodes=pd.c.nodes,
                                                 kern.nodes=kern.nodes,lam.nodes=lam.nodes,
                                                 y.mark.p.nodes=y.mark.p.nodes,y.mark.c.nodes=y.mark.c.nodes,
...
#                   type = 'sSampler',control=list(i=i,J.sight=J.sight,res=data$res,n.cells.x=data$n.cells.x,n.cells.y=data$n.cells.y,
...
data$N
data$n.cap #number of individuals detected in sighting data
