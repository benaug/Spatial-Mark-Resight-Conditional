...
sim.SMR.Dcov <-
  function(D.beta0=NA,D.beta1=NA,D.cov=NA,InSS=NA,res=NA,n.marked=NA,lam0=NA,
           phi=NA,sigma=NA,K=NA,X=X,xlim=NA,ylim=NA,
    ...
    if(obstype=="poisson"){
      ...
      if(is.na(phi))stop("Must provide phi for negbin obstype")
      lamd <- lam0*exp(-D*D/(2*sigma*sigma))
      for(i in 1:N){
        for(j in 1:J){
          y[i,j,1:K1D[j]] <- rnbinom(K1D[j],mu=lamd[i,j],size=phi)
        }
      } 
    }else{
    ...
    n.cap <- sum(rowSums(y)>0)
    
    out <- list(y.mID=y.mID,y.mnoID=y.mnoID,y.um=y.um,y.unk=y.unk, #observed data
                n.marked=n.marked,locs=locs,n.M=n.M,n.UM=n.UM,n.cap=n.cap,
                y=y,y.event=y.event,s=s,#true data
                X=X,K=K,K1D=K1D,
