...
#This version uses a Negative Binomial observation model with a single dispersion (size) parameter, phi
#can be modified so that phi is a function of individual or trap covariates.
#Note: the overdispersion parameter may not be identifiable if you do not have large counts for the
#marked and identified detections. So, identifiability is more likely with no/few marked and unknown ID or
#unknown marked status samples. Identifiability more likely the more overdispersion there is.
#As expected, mixing is worse than with Poisson observtion model. Need to run more iterations.
...
source("NimbleModel SMR Negbin Dcov DA2 Conditional.R")
source("NimbleFunctions SMR Negbin Dcov DA2 Conditional.R")
...
set.seed(143533) #change seed for new data set
data <- sim.SMR.Dcov(D.beta0=D.beta0,D.beta1=D.beta1,res=res,
                ...
                lam0=lam0,phi=phi,sigma=sigma,K=K,X=X,xlim=xlim,ylim=ylim,tlocs=tlocs,
                obstype=obstype)
...
Niminits <- list(y.true=nimbuild$y.true,y.event=nimbuild$y.event,capcounts=rowSums(nimbuild$y.true),
                 ...
                 theta.unmarked=c(0,0.5,0.5),lam0=inits$lam0,sigma=inits$sigma,phi=inits$phi)
...
parameters <- c('D0','lambda.N','lam0','sigma','phi','theta.marked','theta.unmarked',
                'N','D.beta1',"n.cap")
...
config.nodes <- c('lam0','sigma','phi','theta.marked','theta.unmarked[2:3]')
...
conf$addSampler(target = paste("N"),
                type = 'zSampler',control = list(z.ups=z.ups,J=J,n.marked=n.marked,M=M,N.node=N.node,lam.nodes=lam.nodes,
                                                 y.nodes=y.nodes,z.nodes=z.nodes,calcNodes=calcNodes),
                silent = TRUE)
...
plot(mcmc(mvSamples[250:nrow(mvSamples),])) #might take a bit for phi to converge, if it is identifiable.

data$N
data$n.cap #number of individuals detected

sum(Cmodel$logProb_y.true)
Cmodel$calculate()
sum(Cmodel$logProb_y.true)
