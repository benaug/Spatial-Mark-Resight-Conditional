...
IDSampler <- nimbleFunction(
  ...
  run = function() {
    ...
    ll.y <- matrix(0,nrow=M,ncol=J.sight)
    ll.y.event <- matrix(0,nrow=M,ncol=J.sight)
    for(i in 1:M){
      if(z[i]==1){
        for(j in 1:J.sight){
          #if phi is a function of individual or trap covariates, need to fix the lines below, e.g., size=model$phi[i,j]*model$K1D.sight[j]
          p <- model$phi[1]/(model$phi[1] + model$lam[i,j])
          ll.y[i,j] <- dnbinom(y.true[i,j],p=p,size=model$phi[1]*model$K1D.sight[j], log = TRUE)
        }
      }
    }
    for(i in 1:n.marked){
    ...
    for(l in 1:n.samples){ #for all latent ID samples
      ...
      if(ID.cand[l]!=ID.curr[l]){ #abort if we propose the same individual, nothing changes
        ...
        #if phi is a function of individual or trap covariates, need to fix these 2 lines below, e.g., size=model$phi[swapped[1],this.j[l]]*model$K1D.sight[this.j[l]]
        ...
        ll.y.cand[swapped[1],this.j[l]] <- dnbinom(y.true.cand[swapped[1],this.j[l]],
                                                   p=p,size=model$phi[1]*model$K1D.sight[this.j[l]],log=TRUE)
    ...
    for(up in 1:z.ups){ #how many updates per iteration?
      ...
      if(updown==0){ #subtract
        ...
        if(n.z.on>0){ #skip if no unmarked z's to turn off, otherwise nimble will crash
          ...
          #prereject turning off marked individuals or any individuals currently allocated samples
          if(model$capcounts[pick]>0|pick<=n.marked){#is this an marked individual with samples?
            reject <- TRUE
          }
          if(!reject){
            ...
            lp.initial.y.sight <- model$getLogProb(y.sight.nodes[pick])
            ...
            lp.proposed.y.sight <- model$calculate(y.sight.nodes[pick])

            #MH step
            log_MH_ratio <- (lp.proposed.N + lp.proposed.y.mark.p + lp.proposed.y.mark.c + lp.proposed.y.sight) - 
              (lp.initial.N + lp.initial.y.mark.p + lp.initial.y.mark.c + lp.initial.y.sight)
            accept <- decide(log_MH_ratio)
            if(accept) {
              ...
              model$calculate(y.sight.nodes[pick])
