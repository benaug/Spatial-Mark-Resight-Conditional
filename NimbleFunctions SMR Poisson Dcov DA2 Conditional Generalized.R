...
IDSampler <- nimbleFunction(
  ...
  run = function() {
    ...
    for(i in 1:M){
      if(z[i]==1){
        for(j in 1:J.sight){
          ll.y[i,j] <- dpois(y.true[i,j],model$K1D.sight[j]*model$lam[i,j],log=TRUE)
        }
      }
    }
    for(i in 1:n.marked){
    ...
    for(l in 1:n.samples){ #for all latent ID samples
      ...
      if(ID.cand[l]!=ID.curr[l]){ #abort if we propose the same individual, nothing changes
        ...
        ll.y.cand[swapped,this.j[l]] <- dpois(y.true.cand[swapped,this.j[l]],model$lam[swapped,this.j[l]]*model$K1D.sight[this.j[l]],log=TRUE)
